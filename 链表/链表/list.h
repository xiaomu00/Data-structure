...
//////////////////////////////////////////////////////////////////////
//单链表的函数接口声明
static BuyNode(ElmeType x);//开辟一个节点空间并赋值
void listInit(slist *pst);//单链表序表初始化函数声明
void slistPushBackPushBack(slist *plist, ElmeType x);//尾部插入
void slistPushFront(slist *plist, ElmeType x);//头部插入
...
//开辟一个节点空间并赋值
static SListNode* _BuyNode(ElmeType x)
{
...
//尾部插入 时间复杂度为O（n）
void slistPushBackPushBack(slist *plist, ElmeType x)
{
	assert(plist!=NULL);
...
//头部插入  时间复杂度为O（1）
void slistPushFront(slist *plist, ElmeType x)
{
	assert(plist!=NULL);
	SListNode *s = _BuyNode(x);
	s->next = plist->head;
